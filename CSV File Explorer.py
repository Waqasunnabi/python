...
def handle_errors(func):
    def wrapper(*args, **kwargs):
        try:
            ...
            messagebox.showerror("Error", "The specified file or folder could not be found.")
        except UnicodeDecodeError:
            messagebox.showerror("Error", "The specified file cannot be read. Please ensure that the encoding is correct.")
...
def findvalue():
    ...
    if var.get() == "Select Column Name":
        ...
        for idx, filename in enumerate(all_files):
            ...
            progress_bar_per.config(text="{}%".format(int(100 * progress_var.get() / total_loops)))
            progress_bar.update()

            step_execution_time = time.time() - start_time
            total_execution_time += step_execution_time

            step_remaining_time = total_execution_time * (total_loops - idx - 1) / (idx + 1)

            minutes, seconds = divmod(step_remaining_time, 60)
            if progress_var.get() != total_loops:
                estimated_time_label.config(text="Estimated Time Remaining: {:02.0f}:{:02.0f}".format(minutes, seconds))
        ...
        for i, line in enumerate(lines):
            start_time = time.time()
            result_df = pd.concat([result_df, df[df[var.get()] == line]], axis=0, ignore_index=True)
            ...
            step_remaining_time = total_execution_time * (total_loops - len(all_files) - i - 1) / (i + 1)
...
if __name__ == "__main__":
    ...
    style = ttk.Style()
    style.configure("My.TLabel", font=("Arial", 12), background="#353535", foreground="#333333")
    style.configure("My.TButton", font=("Arial", 10, "bold"), background="#353535", foreground="black", borderwidth=0, borderradius=5)
    ...
    path_label = tk.Label(path_frame, text="CSV Path:", font=("Arial", 10), bg="#353535",fg="#fafafa", padx=7)
    ...
    refresh_button = ttk.Button(form_frame, text="Refresh", style="My.TButton", padding=(5, 5), command=start_columnname_thread)
    ...
    text_area = scrolledtext.ScrolledText(text_frame, wrap=tk.WORD, font=("Arial", 10),bg="#353532", fg="white")
    ...
    # Add a "Run" button
    run_button = ttk.Button(button_frame, text="Run", style="My.TButton", padding=(5, 5), command=start_findvalue_thread)
    ...
    progress_bar_per = tk.Label(progress_frame, text="0%", font=("Arial", 10), bg="#353535",fg="#fafafa", padx=7)
    ...
    estimated_time_label = tk.Label(progress_frame, text="Estimated Time Remaining: --",background="#353535",foreground="White")
